...
/** {@link ContextStrategyFactory} that selects the strategy from {@code reviewer.context.strategy}. */
@Component
public class ConfigContextStrategyFactory implements ContextStrategyFactory {

    private static final Logger log = LoggerFactory.getLogger(ConfigContextStrategyFactory.class);

    private final GitLabClient gitlab;
    private final ReviewerProperties props;

    public ConfigContextStrategyFactory(GitLabClient gitlab, ReviewerProperties props) {
        this.gitlab = gitlab;
        this.props  = props;
    }
